...
const statusUpdatesMock = (ws) => () => {
  const statusData = {total_locked_altm: "1000"};
  if (Math.random() > 0.5) {
    statusData['notices'] = [{text: 'rgwgwgweg', color_t: '', color_b: ''}];
  }
  if (Math.random() > 0.1) {
    statusData['banners'] = [
      {
        url_im: 'https://i.imgur.com/tGdKTJX.gif',
        url_cl: '/profile',
        text_a: 'wefewobfowe fewfwfe w',
        internal: true,
      },
      {
        url_im: 'https://i.imgur.com/D0K0aWu.png',
        url_cl: '/fees',
        text_a: 'test',
        internal: true,
      },
  ...
  sendEvent(ws, "global.status_updates", statusData);
};

const balancesMock = (ws) => () => {
  const balancesPayload = Helpers.getBalances();
  for (const coin of Object.keys(balancesPayload)) {
    const balance = balancesPayload[coin];
    balancePayload = {
      currency: coin,
      balance: balance[0],
      locked: balance[1],
    ...
    if (balance.length > 2) {
      balancePayload['locked_for_tier'] = balance[2];
    }
    sendEvent(ws, "balance", balancePayload);
  }
};
...
class RangerMock {
  ...
  initConnection(ws, request) {
    ...
    ws.sequences = {};
        ...
        ws.timers.push(setInterval(statusUpdatesMock(ws, this.markets), 5000));
